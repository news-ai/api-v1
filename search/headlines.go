...
func searchHeadline(elasticQuery interface{}, stringFeeds []string, feedUrls []models.Feed, checkMap bool) ([]Headline, int, error) {
	hits, err := elasticHeadline.QueryStruct(elasticQuery)
	if err != nil {
		log.Printf("%v", err)
...
func SearchHeadlinesByPublicationId(r *http.Request, publicationId int64) ([]Headline, int, error) {
	...
	return searchHeadline(elasticQuery, []string{}, []models.Feed{}, false)
}
